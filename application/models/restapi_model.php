...
class restapi_model extends CI_Model
{
    ...
    public function getallcompetitiondetail($competition, $user)
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            if ($compi->detail->status == '2') {
                $query2 = $this->db->query("SELECT `parti`.`id`,`parti`.`name`,`score`.`score` FROM `competo_competitionparticipant` as `parti` LEFT OUTER JOIN `competo_competitionscore` as `score` ON `score`.`competitionparticipant` = `parti`.`id` AND `score`.`user`='$user' GROUP BY `parti`.`id`");
    ...
    {
        $query=$this->db->query("SELECT * FROM `competo_competitionscore` WHERE `user`='$user' AND `competitionparticipant` = '$participant'");
        if($query->num_rows() > 0) {
          $query=$query->row();
          $id=$query->id;
